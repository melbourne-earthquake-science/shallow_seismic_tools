...
def process_and_save_stacks(data_dir, output_dir, groups, 
                            overwrite=False, save_mseed=True, 
                            save_tab=True, save_su=True, save_segy=True):
    """
    ...
    - groups (list): A list of grouped shots to be processed.
    ...
    for i, group in enumerate(groups):
        ...
        sub_dir = os.path.join(output_dir, f"source{i+1}")
        ...
        # Placeholder for stacking and saving logic
        ...
        for rec, chan, source_distance in geophone_array.iter_reverse():
            stacked = stack_channel(group, ref_time, recorder=rec, channel=chan, base_directory=data_dir, order=1, endbuffer=1, startbuffer=0)
            trace = stacked[0]
        ...
        if save_mseed:
            mseed_file = os.path.join(sub_dir, "stacked_stream.mseed")
            stacked_stream.write(mseed_file, format="MSEED")
            print(f"Saved stacked stream to {mseed_file}")

        if save_tab:
            tab_file = os.path.join(sub_dir, "stacked_stream.txt")
        ...
        if save_su:
            ...
            su_file = os.path.join(sub_dir, "stacked_stream.su")
            stacked_stream.write(su_file, format="SU")
            print(f"Saved stacked stream to {su_file}")

        if save_segy:
            segy_file = os.path.join(sub_dir, "stacked_stream.sgy")
